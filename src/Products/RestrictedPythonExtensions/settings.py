...
SETTINGS_ATTRIBUTE = "_restricted_python_extensions_settings"
LEGACY_SETTINGS_ATTRIBUTE = "_enable_scripts_settings"
...
def get_settings(app, create=False):
    ...
    if settings is None:
        settings = getattr(app, LEGACY_SETTINGS_ATTRIBUTE, None)
        if settings is not None and create:
            # Keep the legacy attribute so old ZODB revisions remain readable,
            # while all future access uses the renamed product attribute.
